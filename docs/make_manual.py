...
TR = {
    "en": {
        "sub1": "The universal translator into the language of AI", "sub2": "Complete manual — users & administrators",
        ...
        "more": ["Audio & video are transcribed locally with Whisper; pick the language under Advanced options.",
                 ...
                 "Visual redaction: download your PDF or scanned image with the PII blacked out on the page — true redaction: the text and the pixels underneath are removed from the file. The document's metadata (title, author, keywords) is wiped too, so nothing leaks via the file's Properties.",
                 "Queue: convert everything or only the selected files.",
                 "Light/dark theme and an interface available in 7 languages.",
                 "Export beyond Markdown: one unified menu turns the result into Word (.docx), ODT, EPUB, HTML, LaTeX, reStructuredText or structured XML (DocBook, JATS, TEI, OPML), via Pandoc — no AI involved.",
                 "LLM prep panel on every conversion: token count, tokens and cost saved by anonymization, a live per-model cost estimate (OpenRouter pricing), context-window fit, one-click RAG chunking and a prompt-injection detector — all local.",
                 "Advanced PDF extraction (optional): the OpenDataLoader engine reconstructs complex layouts with better reading order and heading hierarchy, falling back to the default extractor automatically.",
                 "Page selection (PDF): convert only the pages you need — a range (5-67), individual pages (1, 6, 9) or a mix (1, 2, 5-67), chosen per file with a simple picker that shows the page count."],
        ...
        "api": [
            ...
            "/api/export|POST|Convert Markdown to docx, odt, epub, html, latex, rst or XML (DocBook/JATS/TEI/OPML).",
            "/api/compact|POST|Whitespace-stripped Markdown to save tokens.",
            "/api/chunk|POST|Token-bounded RAG chunks (.jsonl).",
            "/api/model_prices|GET|Live model pricing and context windows (OpenRouter, cached).",
        ],
    ...
    "es": {
        "sub1": "El traductor universal al idioma de la IA", "sub2": "Manual completo — usuarios y administradores",
        ...
        "more": ["El audio y el video se transcriben localmente con Whisper; elige el idioma en Opciones avanzadas.",
                 ...
                 "Censura visual: descarga tu PDF o imagen escaneada con el PII tachado sobre la página — redacción real: el texto y los píxeles de abajo se eliminan del archivo. También se borran los metadatos del documento (título, autor, palabras clave), para que nada se filtre por las Propiedades del archivo.",
                 "Cola: convierte todo o solo los archivos seleccionados.",
                 "Tema claro/oscuro e interfaz disponible en 7 idiomas.",
                 "Exporta más allá de Markdown: un menú unificado convierte el resultado a Word (.docx), ODT, EPUB, HTML, LaTeX, reStructuredText o XML estructurado (DocBook, JATS, TEI, OPML), mediante Pandoc — sin IA.",
                 "Panel de preparación para LLM en cada conversión: recuento de tokens, tokens y costo ahorrados por la anonimización, una estimación de costo en vivo por modelo (precios de OpenRouter), encaje en la ventana de contexto, fragmentación RAG en un clic y un detector de inyección de prompts — todo local.",
                 "Extracción avanzada de PDF (opcional): el motor OpenDataLoader reconstruye diseños complejos con mejor orden de lectura y jerarquía de títulos, y recurre automáticamente al extractor por defecto.",
                 "Selección de páginas (PDF): convierte solo las páginas que necesitas — un rango (5-67), páginas sueltas (1, 6, 9) o una mezcla (1, 2, 5-67), elegidas por archivo con un selector simple que muestra la cantidad de páginas."],
        ...
        "api": [
            ...
            "/api/export|POST|Convierte Markdown a docx, odt, epub, html, latex, rst o XML (DocBook/JATS/TEI/OPML).",
    ...
    "fr": {
        "sub1": "Le traducteur universel vers le langage de l'IA", "sub2": "Manuel complet — utilisateurs et administrateurs",
        ...
        "more": ["L’audio et la vidéo sont transcrits en local avec Whisper ; choisissez la langue dans les options avancées.",
                 ...
                 "Caviardage visuel : téléchargez votre PDF ou image scannée avec les PII noircies sur la page — caviardage réel : le texte et les pixels en dessous sont supprimés du fichier. Les métadonnées du document (titre, auteur, mots-clés) sont également effacées, pour que rien ne fuite via les Propriétés du fichier.",
                 ...
                 "Sélection de pages (PDF) : convertissez uniquement les pages dont vous avez besoin — une plage (5-67), des pages individuelles (1, 6, 9) ou un mélange (1, 2, 5-67), choisies fichier par fichier avec un sélecteur simple qui affiche le nombre de pages."],
    ...
    "pt": {
        "sub1": "O tradutor universal para o idioma da IA", "sub2": "Manual completo — usuários e administradores",
        ...
        "more": ["Áudio e vídeo são transcritos localmente com Whisper; escolha o idioma nas opções avançadas.",
                 ...
                 "Censura visual: baixe seu PDF ou imagem digitalizada com os dados tachados na página — redação real: o texto e os pixels abaixo são removidos do arquivo. Os metadados do documento (título, autor, palavras-chave) também são apagados, para que nada vaze pelas Propriedades do arquivo.",
    ...
    "it": {
        "sub1": "Il traduttore universale verso il linguaggio dell'IA", "sub2": "Manuale completo — utenti e amministratori",
        ...
        "more": ["Audio e video sono trascritti in locale con Whisper; scegli la lingua nelle opzioni avanzate.",
                 ...
                 "Censura visiva: scarica il PDF o l’immagine scansionata con i dati oscurati sulla pagina — redazione reale: il testo e i pixel sottostanti vengono rimossi dal file. Vengono cancellati anche i metadati del documento (titolo, autore, parole chiave), così nulla trapela tramite le Proprietà del file.",
                 "Coda: converti tutto o solo i file selezionati.",
                 "Tema chiaro/scuro e interfaccia disponibile in 7 lingue.",
                 "Esportazione oltre Markdown: un menu unificato converte il risultato in Word (.docx), ODT, EPUB, HTML, LaTeX, reStructuredText o XML strutturato (DocBook, JATS, TEI, OPML), tramite Pandoc — senza IA.",
                 "Pannello di preparazione per LLM a ogni conversione: conteggio dei token, token e costo risparmiati dall’anonimizzazione, una stima del costo in tempo reale per modello (prezzi OpenRouter), adattamento alla finestra di contesto, suddivisione RAG con un clic e un rilevatore di prompt injection — tutto in locale.",
                 "Estrazione PDF avanzata (opzionale): il motore OpenDataLoader ricostruisce layout complessi con un migliore ordine di lettura e gerarchia dei titoli, ricorrendo automaticamente all’estrattore predefinito.",
                 "Selezione delle pagine (PDF): converti solo le pagine che ti servono — un intervallo (5-67), pagine singole (1, 6, 9) o un misto (1, 2, 5-67), scelte per ogni file con un semplice selettore che mostra il numero di pagine."],
        ...
        "api": [
            ...
            "/api/export|POST|Converte Markdown in docx, odt, epub, html, latex, rst o XML (DocBook/JATS/TEI/OPML).",
            "/api/compact|POST|Markdown senza spazi superflui per risparmiare token.",
            "/api/chunk|POST|Frammenti RAG limitati per token (.jsonl).",
    ...
    "zh": {
        "sub1": "通往 AI 语言的通用翻译器", "sub2": "完整手册 —— 用户与管理员",
        ...
        "more": ["音频和视频使用 Whisper 在本地转写；在高级选项中选择语言。",
                 ...
                 "可视化遮蔽：下载 PII 在页面上被涂黑的 PDF 或扫描图片 — 真正的涂黑：文字与底层像素都从文件中删除。文档的元数据（标题、作者、关键词）也会一并清除，因此不会通过文件的「属性」泄露。",
                 "队列：可转换全部或仅所选文件。",
                 "浅色/深色主题，界面提供 7 种语言。",
                 "超越 Markdown 的导出：一个统一菜单可通过 Pandoc 将结果转换为 Word (.docx)、ODT、EPUB、HTML、LaTeX、reStructuredText 或结构化 XML（DocBook、JATS、TEI、OPML）—— 不涉及 AI。",
                 "每次转换都有 LLM 准备面板：token 计数、匿名化节省的 token 与费用、按模型的实时费用估算（OpenRouter 定价）、上下文窗口适配、一键 RAG 分块和提示词注入检测器 —— 全部本地完成。",
                 "高级 PDF 提取（可选）：OpenDataLoader 引擎以更好的阅读顺序与标题层级重建复杂版面，并在需要时自动回退到默认提取器。",
                 "页面选择（PDF）：只转换你需要的页面 —— 一个范围（5-67）、单独页面（1、6、9）或混合（1、2、5-67），可按文件用简单的选择器挑选，并会显示页数。"],
        "p2": "第 2 部分 —— 角色与访问级别",
        ...
        "api": [
            ...
            "/api/export|POST|将 Markdown 转换为 docx、odt、epub、html、latex、rst 或 XML（DocBook/JATS/TEI/OPML）。",
            "/api/compact|POST|去除多余空白的 Markdown，以节省 token。",
            "/api/chunk|POST|按 token 限定的 RAG 分块（.jsonl）。",
    ...
    "ja": {
        "sub1": "AI の言語への、ユニバーサル翻訳機", "sub2": "完全マニュアル —— 利用者と管理者向け",
        ...
        "more": ["音声・動画は Whisper でローカルに文字起こし。詳細オプションで言語を選択。",
                 ...
                 "ビジュアル墨消し：PII をページ上で黒塗りした PDF・スキャン画像をダウンロード — 本物の墨消し：テキストと下のピクセルはファイルから削除されます。文書のメタデータ（タイトル、作成者、キーワード）も消去されるため、ファイルの「プロパティ」から漏れることもありません。",
                 "キュー：すべて、または選択したファイルのみを変換。",
                 "ライト/ダークテーマ、UI は 7 言語に対応。",
                 "Markdown を超えるエクスポート：統一メニューから、Pandoc を介して結果を Word (.docx)、ODT、EPUB、HTML、LaTeX、reStructuredText、または構造化 XML（DocBook、JATS、TEI、OPML）に変換 —— AI は不要。",
                 "変換ごとに LLM 準備パネル：トークン数、匿名化で節約できたトークンとコスト、モデル別のリアルタイムなコスト見積もり（OpenRouter 価格）、コンテキストウィンドウへの収まり、ワンクリックの RAG チャンク分割、プロンプトインジェクション検出器 —— すべてローカルで。",
                 "高度な PDF 抽出（任意）：OpenDataLoader エンジンが、より良い読み順と見出し階層で複雑なレイアウトを再構築し、必要に応じて既定の抽出器へ自動的にフォールバックします。",
                 "ページ選択（PDF）：必要なページだけを変換 — 範囲（5-67）、個別ページ（1、6、9）、またはその組み合わせ（1、2、5-67）を、ページ数を表示するシンプルなピッカーでファイルごとに選べます。"],
        ...
        "api": [
            ...
            "/api/export|POST|Markdown を docx、odt、epub、html、latex、rst または XML（DocBook/JATS/TEI/OPML）に変換。",
            "/api/compact|POST|余分な空白を除いた Markdown でトークンを節約。",
            "/api/chunk|POST|トークン上限付きの RAG チャンク（.jsonl）。",
            "/api/model_prices|GET|モデルのリアルタイム価格とコンテキストウィンドウ（OpenRouter、キャッシュ済み）。",
        ],
