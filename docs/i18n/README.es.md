...
- 📤 **Exporta a 13 formatos** — más allá de Markdown, un único menú unificado exporta el resultado a **Word (.docx)**, ODT, EPUB, HTML, LaTeX, reStructuredText y **XML** estructurado (DocBook, JATS, TEI, OPML) mediante [Pandoc](https://pandoc.org/), además de los formatos de datos **JSON**, **YAML** y **[TOON](https://github.com/toon-format/toon)** (compacto y eficiente en tokens para LLMs). Sin IA de por medio.
- 🔊 **Texto a audio (podcast)** — convierte el documento ya convertido en un **MP3**: una **narración** a una sola voz o un **podcast a dos voces** guionado por IA. Voces locales de [Piper](https://github.com/rhasspy/piper) (sin conexión, 14 voces en es/en/pt/fr/it/de/zh) o voces opcionales en la nube de **OpenAI**, con controles de tono / velocidad / volumen.
- ✏️ **Editor de Markdown integrado** — abre el resultado en un **editor a pantalla completa con vista previa en vivo** para depurarlo antes de exportarlo o convertirlo a voz. Tus ediciones se reflejan en todas las salidas (Word, XML, MP3…).
