...
TR = {
    "en": {
        ...
        "more": ["Audio & video are transcribed locally with Whisper; pick the language under Advanced options.",
                 ...
                 "PII anonymization for LLMs: a local engine (NER + invoice layout + validated detectors + your own rules) with typed, anonymous or reversible pseudonymization (token → send to the LLM → re-hydrate).",
    ...
    "es": {
        ...
        "more": ["El audio y el video se transcriben localmente con Whisper; elige el idioma en Opciones avanzadas.",
                 ...
                 "Anonimización de PII para LLMs: motor local (NER + layout de facturas + detectores validados + reglas propias) con salida tipada, anónima o seudonimizada reversible (token → al LLM → re-hidratás).",
    ...
    "fr": {
        ...
        "more": ["L’audio et la vidéo sont transcrits en local avec Whisper ; choisissez la langue dans les options avancées.",
                 ...
                 "Anonymisation des PII pour LLM : moteur local (NER + structure des factures + détecteurs validés + vos règles) avec sortie typée, anonyme ou pseudonymisée réversible (jeton → au LLM → ré-hydratation).",
    ...
    "pt": {
        ...
        "more": ["Áudio e vídeo são transcritos localmente com Whisper; escolha o idioma nas opções avançadas.",
                 ...
                 "Anonimização de PII para LLMs: motor local (NER + layout de faturas + detectores validados + regras próprias) com saída tipada, anônima ou pseudonimizada reversível (token → ao LLM → re-hidratar).",
    ...
    "it": {
        ...
        "more": ["Audio e video sono trascritti in locale con Whisper; scegli la lingua nelle opzioni avanzate.",
                 ...
                 "Anonimizzazione PII per LLM: motore locale (NER + layout fatture + rilevatori validati + regole proprie) con output tipizzato, anonimo o pseudonimizzato reversibile (token → all’LLM → re-idratazione).",
    ...
    "zh": {
        ...
        "more": ["音频和视频使用 Whisper 在本地转写；在高级选项中选择语言。",
                 ...
                 "为 LLM 的 PII 匿名化：本地引擎（NER + 发票版面 + 校验检测器 + 自定义规则），可选带类型、匿名或可逆假名化（占位符 → 发给 LLM → 还原）。",
                 "队列：可转换全部或仅所选文件。",
                 "浅色/深色主题，界面提供 7 种语言。"],
        "p2": "第 2 部分 —— 角色与访问级别",
    ...
    "ja": {
        ...
        "more": ["音声・動画は Whisper でローカルに文字起こし。詳細オプションで言語を選択。",
                 ...
                 "LLM 向け PII 匿名化：ローカルエンジン（NER + 請求書レイアウト + 検証付き検出器 + 独自ルール）。型付き・匿名・可逆な仮名化（トークン → LLM → 復元）に対応。",
