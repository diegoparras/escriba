...
- 📤 **Export vers 13 formats** — au‑delà du Markdown, un menu unifié exporte le résultat vers **Word (.docx)**, ODT, EPUB, HTML, LaTeX, reStructuredText et **XML** structuré (DocBook, JATS, TEI, OPML) via [Pandoc](https://pandoc.org/), ainsi que les formats de données **JSON**, **YAML** et **[TOON](https://github.com/toon-format/toon)** (compact et économe en tokens pour les LLM). Sans aucune IA.
