...
- 📤 **Esportazione in 13 formati** — oltre al Markdown, un unico menu esporta il risultato in **Word (.docx)**, ODT, EPUB, HTML, LaTeX, reStructuredText e **XML** strutturato (DocBook, JATS, TEI, OPML) tramite [Pandoc](https://pandoc.org/), più i formati di dati **JSON**, **YAML** e **[TOON](https://github.com/toon-format/toon)** (compatto ed efficiente in termini di token per gli LLM). Nessun LLM coinvolto.
